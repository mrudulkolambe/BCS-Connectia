...
import SmallSolutionRoutes from "./SmallSolutionRoutes";
import SolutionRoutes from "./SolutionRoutes";

export default function Navbar() {
  const [show, setShow] = useState(false)
  function sideBar() {
    const menu = document.getElementById("navbar-menubar");
  ...
  return (
    <>
      ...
      <nav className="relative lg:px-24 px-10 py-6 flex justify-between items-center bg-black min-w-screen">
        ...
        <ul className="hidden absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 lg:flex lg:mx-auto lg:items-center lg:w-auto lg:space-x-6">
          ...
          <li className="nav-item relative cursor-pointer">
            <a onClick={() => { setShow(!show) }} className="nav-link">
              Solution
            </a>
          </li>
          <li className="nav-item">
      ...
      <div className="navbar-menu relative z-50 hidden w-full" id="navbar-menubar">
        <div className="navbar-backdrop fixed inset-0 bg-gray-800 opacity-25"></div>
        <nav className="fixed top-0 right-0 bottom-0 flex flex-col w-full max-w-sm py-6 px-6 bg-black border-r overflow-y-auto">
          <div className="flex items-center mb-8">
            <Link href="/" className="w-48">
              <img src="/logo.png" className="w-full" />
            </Link>
            <div className="w-10 ml-20">
              <RiCloseFill color="white" className="w-full h-auto cursor-pointer" onClick={() => {
                sideBar();
              }} />
            </div>
          </div>
          <div>
            <ul>
              ...
              <li className="mb-1 nav-item">
                <a
                  className="cursor-pointer block p-4 text-sm font-semibold text-white hover:bg-blue-50 hover:text-blue-600 rounded"
                  onClick={() => { setShow(!show) }}
                >
                  Solution
                </a>
              </li>
              <SmallSolutionRoutes show={show} setShow={setShow} />
